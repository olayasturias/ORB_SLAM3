/**
* ORB-SLAM3 example for the EIVA dataset (stereo).
*
* Dataset structure expected:
*   <sequence_root>/processed/left/   — left images *.jpg (Zephyr filenames)
*   <sequence_root>/processed/right/  — right images *.jpg (same naming)
*
* Zephyr filename formats (both are handled):
*   YYYY-MM-DDTHHMMSS.fffZ.jpg        — compact time, UTC Z suffix
*   YYYY-MM-DDTHH-MM-SS-fff.jpg       — dashed time with milliseconds
*
* Usage:
*   stereo_eiva path_to_vocabulary path_to_settings path_to_sequence [trajectory_name]
*/
...
int main(int argc, char **argv)
{
    if(argc < 4 || argc > 5)
    {
        cerr << "\nUsage: ./stereo_eiva path_to_vocabulary path_to_settings"
                " path_to_sequence [trajectory_name]\n"
             << "  path_to_sequence : folder containing processed/left/ and processed/right/\n";
        return 1;
    }
    ...
    vector<double> vTimestamps;
    LoadImages(seqPath + "/processed/left",
               seqPath + "/processed/right",
               vstrImageLeft, vstrImageRight, vTimestamps);

    const int nImages = (int)vstrImageLeft.size();
    if(nImages == 0){ cerr << "No images found in " << seqPath << "/processed/left\n"; return 1; }
                ...
                vector<double> &vTimeStamps)
{
    ...
    cv::glob(strPathLeft  + "/*.jpg", filesL, false);
    cv::glob(strPathRight + "/*.jpg", filesR, false);
