/**
* ORB-SLAM3 example for the TartanAir2 dataset (monocular, left-front camera).
*
* Dataset structure expected:
*   <sequence_root>/image_lcam_front/  — images named *.png (sorted)
*   <sequence_root>/imu/cam_time.txt   — optional; one timestamp (seconds) per line.
*                                        If absent, 10 Hz is assumed.
*
* Usage:
*   mono_tartanair2 path_to_vocabulary path_to_settings path_to_sequence [trajectory_name]
*/
...
int main(int argc, char **argv)
{
    if(argc < 4 || argc > 5)
    {
        cerr << "\nUsage: ./mono_tartanair2 path_to_vocabulary path_to_settings"
                " path_to_sequence [trajectory_name]\n"
             << "  path_to_sequence : folder containing image_lcam_front/ "
    ...
    LoadImages(string(argv[3]) + "/image_lcam_front",
               string(argv[3]) + "/imu/cam_time.txt",
               vstrImages, vTimestamps);

    const int nImages = (int)vstrImages.size();
    if(nImages == 0){ cerr << "No images found in " << string(argv[3]) << "/image_lcam_front\n"; return 1; }
